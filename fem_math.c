...
int femSparseMatPrnF(char *fname, tMatrix *mat)
{
  ...
  fprintf(fw, "%li %li %li\n", mat->rows, mat->cols, mat->len);
	for (i=0; i<mat->rows; i++) { fprintf(fw, "%li %li \n", mat->frompos[i], mat->defpos[i]) ; }
	for (i=0; i<mat->len; i++) { fprintf(fw, "%li %e \n", mat->pos[i], mat->data[i]) ; }
#else
  fprintf(fw, "%li %li\n", mat->rows, mat->cols);
...
int femVecPutAdd(tVector *vec, long pos, double val, int mode)
{
	...
	{
#ifdef RUN_VERBOSE
		fprintf(msgout,"[E] Vadd: %s: %li!\n", _("Index outside vector"),pos);
#endif
		return(AF_ERR_BIG);
	}
...
double femVecGet(tVector *vec, long pos)
{
	...
	{
#ifdef RUN_VERBOSE
		fprintf(msgout,"[E] Vget %s: %li/%li!\n", _("Index outside vector"),pos,vec->rows);
#endif
		return(0);
	}
